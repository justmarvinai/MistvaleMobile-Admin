...
**Phases A0–A1 complete → next is A2 (world authoring)** — see `ROADMAP.md`. The scaffold, typed API layer, rank-gated auth, app shell, dashboard, publish center, the generic entity browser (all **21** content types, dungeons, masteries, quests, missions, events and login tracks included), the Game config / Champion / Skills editors, **Player management** and the **Arena bot manager** exist and are tested. Campaign, drop-table, balance-sandbox and battle-inspector editors do not yet.
