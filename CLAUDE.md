...
**Phases A0–A1 complete → next is A2 (world authoring)** — see `ROADMAP.md`. The scaffold, typed API layer, rank-gated auth, app shell, dashboard, publish center, the generic entity browser (all **23** content types, dungeons, masteries, quests, missions, events, login tracks, news and the tutorial script included), the Game config / Champion / Skills editors, **Player management**, the **Arena bot manager**, the **mail composer** and the **tutorial script editor** exist and are tested. Campaign, drop-table, balance-sandbox and battle-inspector editors do not yet.
