...
Boundary discipline: the game repo owns _data + rules_; this repo owns the _entire operator experience_ (all screens, editors, workflows in ADMIN_SUITE_DESIGN.md). The Admin Suite never touches PostgreSQL directly.
...
| UI              | React 18 + **Mantine 7**                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Internal tool: forms/tables/modals productivity beats hand-crafting; MIT; dark theme; no serif fonts. (The _game_ client stays 100% custom-built — this is deliberately different.) |
...
| Testing         | Vitest + React Testing Library (critical editors: skill composer, publish diff), Playwright smoke, `pnpm e2e` (login → edit → diff → validate → publish → read back through the bundle the game serves, plus a walk of every screen). Outside `pnpm verify`, since it needs the game server, this SPA and an admin account running; the account comes in as `E2E_ADMIN_ACCOUNT`/`E2E_ADMIN_PASSWORD` because there is no self-serve promotion and this repo must not touch the database |                                                                                                                                                                                     |
